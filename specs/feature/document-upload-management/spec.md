...
2. **Given** a user tries to upload a file above the 25 MB limit or with a blocked file type, **When** the upload is submitted, **Then** the system rejects it with a clear validation message and does not create a document record.
3. **Given** a valid upload is submitted, **When** asynchronous virus screening is still pending or later fails, **Then** the document remains unavailable for viewing, download, and sharing; a failed scan is marked quarantined or failed and produces an audit record and user notification.
4. **Given** a user uploads a document associated with a project, **When** they view the project details or project document list, **Then** the document is visible only to authorized project participants and is labeled with the correct project association after it is clean.
5. **Given** a user is on the upload workflow, **When** a large file is being uploaded, **Then** they see a progress indicator and a success or error message at completion.
...
- **FR-017**: The system MUST place uploaded files into a pending-scan state, perform malware and virus scanning asynchronously after upload, and prevent access until the scan succeeds. Files that fail scanning MUST be marked quarantined or failed, excluded from normal document views, and unavailable for download or sharing.
